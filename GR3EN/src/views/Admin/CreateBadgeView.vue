    ...
    <v-main>
      <v-container>
        <v-form @submit.prevent="submitForm" ref="form">
          ...
          <v-file-input ref="file" label="Badge Image" accept="image/*" required></v-file-input>
  ...
  methods: {
    submitForm() {
      if (this.$refs.form.validate()) {
        ...
        axios.post('http://127.0.0.1:3000/badges/', formData,{headers:{Authorization:'Bearer '+localStorage.getItem('Token')}})
          .then(response => {
